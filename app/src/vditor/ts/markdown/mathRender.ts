...
export const mathRender = (element: HTMLElement, options?: { cdn?: string, math?: IMath }) => {
    ...
    if (options.math.engine === 'KaTeX') {
        addStyle(`${options.cdn}/js/katex/katex.min.css`, 'vditorKatexStyle');
        addScript(`${options.cdn}/js/katex/katex.min.js`, 'vditorKatexScript').then(() => {
        ...
        if (!window.MathJax) {
            window.MathJax = {
                loader: {
                    paths: {mathjax: `${options.cdn}/js/mathjax`},
                },
                tex: {
        ...
        // 循环加载会抛异常
        addScriptSync(`${options.cdn}/js/mathjax/tex-svg.js`, 'vditorMathJaxScript');
        // 等到 js 文件执行完成
        setTimeout(() => {
            mathElements.forEach((mathElement) => {
                if (mathElement.getAttribute('data-math')) {
